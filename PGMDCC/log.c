...
bool Log_Writer::loginit(LogLevel l, const  char *filelocation, bool append, bool issync)
{
    ...
    int filelocationlen = strlen(filelocation);
    int filelocationlimitlen = sizeof(m_filelocation);

	if(filelocationlen >= filelocationlimitlen -1 )
	{
		fprintf(stderr, "the path of log file is too long:%d limit:%d\n", filelocationlen, filelocationlimitlen -1);
		exit(0);
	}
